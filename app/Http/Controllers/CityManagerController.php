        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($cityManager)->addIndexColumn()
                ->addColumn('action', function ($user) {
                    $Btn = '<a href="' . route('users.show', $user->user_id) . '" class="view btn btn-info btn-xl mr-3">View</a>';
                    $Btn .= '<a href="' . route('users.show', $user->user_id) . '" class="edit btn btn-primary btn-xl mr-3">Edit</a>';
